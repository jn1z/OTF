 ...
 */
public class ACElts {
    private List<SmartBitSet> elts = new ArrayList<>(2);
    ...
    boolean properSubsetExists(SmartBitSet newElt) {
        ...
        int newEltCard = newElt.cardinality();
    ...
    List<SmartBitSet> getLiveElts() {
        int eltsSize = this.elts.size();
        List<SmartBitSet> allElts = new ArrayList<>(eltsSize - this.deadElts.cardinality());
     ...
     */
    final boolean unifyEltIntoAC(SmartBitSet newElt, int nNFA) {
        // unifySubsetOrReplaceElt() can alter deadElts
        if (!this.unifySubsetOrReplaceElt(newElt)) {
            return false;
        }
        ...
        if (this.invertedIndex == null) {
            ...
            if (!this.deadElts.isEmpty()) {
                ...
                int i = this.deadElts.nextSetBit(0);
                this.invertedIndex.overwrite(newElt, i);
                ...
                this.invertedIndex.insert(newElt);
                this.elts.add((SmartBitSet) newElt.clone());
            }
        }
        return true;
    ...
    private void rebuildInvertedIndex(int nNFA) {
        List<SmartBitSet> rebuildElts = getLiveElts();
        this.deadElts.clear();
     ...
     */
    private boolean unifySubsetOrReplaceElt(SmartBitSet newElt) {
        if (this.invertedIndex != null) {
            // InvertedIndex logic
            ...
            SmartBitSet supersets = invertedIndex.findSupersetIndices(this.elts, this.deadElts, newElt, false);
            if (!supersets.isEmpty()) {
                // newElt is a subset of some current elements. Replace one; the rest are now dead.
                int idxToReplace = supersets.nextSetBit(0);
                this.elts.set(idxToReplace, newElt);
                this.invertedIndex.overwrite(newElt, idxToReplace);
            ...
            int eltSize = elts.size();
            for (int i = 0; i < eltSize; i++) {
                SmartBitSet currentElt = this.elts.get(i);
    ...
    private void replaceAC(SmartBitSet newElt, int i) {
        ...
        this.elts.set(i, newElt);
        ListIterator<SmartBitSet> iterator = this.elts.listIterator(i+1);
        while (iterator.hasNext()) {
            SmartBitSet oldElt = iterator.next();
